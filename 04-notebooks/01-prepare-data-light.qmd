...
df <- read.csv(here(01-data/raw/your-data.csv))
```

## Check and Modify Data Types

**Action Steps:** Start by listing the current data types of variables in your df. This helps identify any variables that may be incorrectly typed, such as numeric variables recognized as character data due to formatting issues.

```{r}
#| label: display_data_structure
#| purl: false

# Display the current data types of all variables in the df
str(df)
...
Before further processing, you may want to conduct an exploratory data analysis (EDA) to gain insights into the df's distribution and characteristics. There are various ways to do this and also packages for automated EDA analysis (e.g., DataExplorer, GGally, SmartEDA, Hmisc). For this tutorial, we will use the DataExplorer.
...
df %>%
    create_report(
        ...
        report_title = "Initial EDA Report - Babysteps df",
...
This step involves identifying and correcting issues in your df, such as missing values, errors, outliers, and standardizing variable names, to ensure data quality.

### Identifying Missing Values

-   **Overview:** Begin by identifying missing values in your df. Missing data can occur for various reasons, from non-response in surveys to errors in data entry.
...
Ensure all variable names are in lowercase to maintain consistency across your df. This can help avoid case-sensitive errors in your analysis scripts.
...
This step encompasses the processes of adjusting your variables to better fit your analysis needs and preparing your data through various transformations. It ensures that your df is in the optimal form for analysis, addressing both the structure of your data and the scales of measurement.
...
-   **Action Steps:** Clearly define your recoding rules and apply them uniformly across your df. Document changes to ensure transparency and reproducibility.
...
After cleaning and transforming your data, perform another round of EDA to verify the data preparation steps' effects and ensure the df is ready for analysis. Look for any remaining issues to check the data quality and structure post-cleaning.
...
df %>%
    create_report(
        ...
        report_title = "EDA Report with processed data - Babysteps df",
        y = 'puzzletime'
    )
message("The EDA report was created and saved in your notebooks folder")
```
...
write.csv(df, here("01-data/processed/my-data.csv"), row.names = FALSE)
